...
import type { Delmatch, DelmatchSerie, DelmatchSummary } from '@/lib/delmatch';
  ...
  return (
    <View>
      ...
      {summary.series.map((s) => (
        <SerieBlock key={s.serie} serie={s} onOpenPlayer={onOpenPlayer} />
      ))}
    </View>
  );
}

function SerieBlock({ serie, onOpenPlayer }: { serie: DelmatchSerie; onOpenPlayer: (id: string) => void }) {
  return (
    <View style={styles.serie}>
      ...
      {serie.tables.map((d) => (
        <BordRow key={d.tableNo} d={d} onOpenPlayer={onOpenPlayer} />
      ))}
    </View>
  );
}

function BordRow({ d, onOpenPlayer }: { d: Delmatch; onOpenPlayer: (id: string) => void }) {
  ...
  return (
    <View style={styles.bord}>
      <View style={[styles.namesCol, { alignItems: 'flex-start' }]}>
        {d.home.map((p, i) => (
          <Fragment key={i}>
            <PlayerName name={p.name} publicId={p.publicId} align="left" onOpenPlayer={onOpenPlayer} />
          </Fragment>
        ))}
      </View>
      ...
      <View style={[styles.namesCol, { alignItems: 'flex-end' }]}>
        {d.away.map((p, i) => (
          <Fragment key={i}>
            <PlayerName name={p.name} publicId={p.publicId} align="right" onOpenPlayer={onOpenPlayer} />
          </Fragment>
        ))}
      </View>
    </View>
...
function PlayerName({
  ...
  return (
    ...
    >
      <Text
        style={[styles.name, { textAlign: align }, tappable && styles.nameLink]}
        numberOfLines={1}
      >
        {name}
      </Text>
