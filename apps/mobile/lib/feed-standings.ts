import { computeStandings, type MatchRow, standingsNeighbors } from '@bowlkollen/core';
...
function divisionStory(matches: MatchRow[], divisionId: number, division: string, historical: boolean): FeedStanding | null {
  ...
  const consider = (ids: number[], requireNotable: boolean) => {
    for (const id of ids) {
      ...
      if (rank === 1) score += 2; // tiebreak toward the top
  ...
  if (!best) consider([table[0].teamId], false); // fallback: the leader
...
async function seasonMatches(divisionId: number, season: number): Promise<MatchRow[]> {
  const { data } = await supabase.from('bits_matches').select(STANDING_COLS).eq('bits_division_id', divisionId).eq('season_id', season);
