...
// A klot's visual: the real bowwwl.com photo when we have one, else a generated glossy
// orb (brand-hashed hue) so the catalog reads well before/without an image.
function hue(s: string) { let h = 0; for (const c of s) h = (h * 31 + c.charCodeAt(0)) % 360; return h }

export function BallOrb({ name, imageUrl, size = 52 }: { name: string; imageUrl?: string | null; size?: number }) {
  if (imageUrl) {
    return (
