...
// feed_reactions + get_feed_reactions aren't in the generated types (run
// supabase/migrations/feed_reactions.sql) — reach them untyped.
...
/** Batch like-count + the caller's liked/saved for the feed's post keys, one RPC. */
export function useFeedReactions(postKeys: string[]) {
  return useQuery({
    ...
    queryFn: async (): Promise<ReactionMap> => {
      const { data, error } = await untyped().rpc('get_feed_reactions', { p_post_keys: postKeys })
...
/** Optimistic like/save: patch every active reactions cache, then persist (revert on fail). */
export function useReactionActions() {
  const qc = useQueryClient()

  const patch = useCallback((key: string, fn: (s: ReactionState) => ReactionState) => {
  ...
  const toggleLike = useCallback((key: string, currentlyLiked: boolean) => {
  ...
  const toggleSave = useCallback((key: string, currentlySaved: boolean) => {
