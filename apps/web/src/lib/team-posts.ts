...
export type TeamPost = {
  id: string; kind: 'message' | 'poll'; title: string | null; body: string
  createdAt: string; authorName: string; isMine: boolean; options: PollOption[]
}
...
export function useTeamPosts(teamId: number) {
  return useQuery({
    ...
    queryFn: async (): Promise<TeamPost[]> => {
      const { data, error } = await untyped().rpc('get_team_posts', { p_bits_team_id: teamId })
...
export function useCreateTeamPost(teamId: number) {
  ...
  return useMutation({
    mutationFn: async (v: { title: string; body: string }) => {
      const { error } = await untyped().rpc('create_team_post', { p_bits_team_id: teamId, p_title: v.title, p_body: v.body })
      if (error) throw error
    },
...
export function useCreateTeamPoll(teamId: number) {
  ...
  return useMutation({
    mutationFn: async (v: { title: string; body: string; options: string[] }) => {
      const { error } = await untyped().rpc('create_team_poll', {
        p_bits_team_id: teamId, p_title: v.title, p_body: v.body,
...
export function useVotePost(teamId: number) {
  ...
  return useMutation({
    mutationFn: async (v: { postId: string; optionId: string }) => {
      const { error } = await untyped().rpc('vote_team_post', { p_post_id: v.postId, p_option_id: v.optionId })
      if (error) throw error
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ['team-posts', teamId] }),
  })
...
export function useMarkPostsSeen(teamId: number) {
  ...
  return useMutation({
    mutationFn: async () => {
      const { error } = await untyped().rpc('mark_team_posts_seen', { p_bits_team_id: teamId })
      if (error) throw error
    },
...
/** Per-team unread counts → the badge on the profile / team page. */
export function useMyUnread() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Record<number, number>> => {
      const { data, error } = await untyped().rpc('get_my_unread_posts')
      if (error) throw error
      const out: Record<number, number> = {}
      for (const r of (data ?? []) as Record<string, unknown>[]) out[r.bits_team_id as number] = (r.unread as number) ?? 0
      return out
    },
  })
}
