...
| **Live scoring** (match in progress) | ✅ `matcher/[id]/_components/LiveScoreboard` (2026-09-12) | 🟡 pending | **Web ahead** — a live match (`isMatchLive`: not finished + within kickoff window) polls `/api/live/[matchId]` (server-side GetMatchScores, ~25s shared cache) every 45s and renders the serie-by-serie board via `MatchResults` with a LIVE pulse + running score. Native: port the hook + a live board onto the native match screen. Note: live population depends on the division being live-scored in BITS; degrades to "waiting for first serie" otherwise. |
