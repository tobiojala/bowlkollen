...
| Anslagstavla (nyheter) | ✅ `/lag/[id]/nyheter` | ✅ `/lag/[id]/nyheter` (`team-posts.ts`) | Both. Web board was orphaned (nothing linked to it) — now surfaced from the team page via LagActions with an unread badge (2026-09-13). |
| Members / role back-office | ✅ `/lag/[id]/medlemmar` | ✅ `TeamBackOffice` | **Parity (2026-09-13).** Web: squad list (`useVerifiedTeamMembers`) + captain per-member role assignment (`useSetMemberRole` → `set_member_role`, needs `team_role_admin.sql` live). Native bundles team colour/header appearance too (web appearance is a later add). |
...
| **Home story rail (entity circles)** | ✅ `StoryRail` (2026-08-27) | ✅ `StoryRail` (2026-08-27) | **Parity** — both: view chips + a lit/seen circle per followed player & team (IG order, tap→filter). Native uses new ID-carrying hooks (`feed-follows.ts`) since `get_user_season_matches` drops IDs; seen-state in SecureStore (web: localStorage). Small diffs to reconcile: native chips are Allt/Matcher/**Serier** vs web's Allt/Matcher/**Prediktion** (native Tipsligan pending); native shows **all** follows as circles, web only those with recent activity. |
...
| Home promo slot | ✅ house card → `/annonsera` (2026-08-27) | ✅ house card → `/annonsera` (2026-08-27) | **Parity** — both show the honest "advertise here" house card (no fake sponsors) + an advertiser screen/page. Native `kind: 'house'` promo + `app/annonsera.tsx`. |
...
| Feature | Web | Native | Notes / gap |
|---|:--:|:--:|---|
| Match page | ✅ `/matcher/[id]` | ✅ `/matcher/[id]` | **Full redesign on both (2026-08-27).** Hero, spelresultat, bord sheet, rivalry, Matchens bästa shared; native gained SeasonContext (H2H+standings), Pro snitt-deltas (spelresultat + bord), Höjdpunkter, UpcomingPanel + the `usePro` seam. Web keeps the wide two-column desktop layout (phone n/a). |
| **Live scoring** (match in progress) | ✅ `matcher/[id]/_components/LiveScoreboard` (2026-09-12) | ✅ `components/LiveScoreboard` (2026-09-12) | **Parity.** A live match (`isMatchLive`: not finished + within kickoff window) polls `/api/live/[matchId]` (server-side GetMatchScores, ~25s shared cache) every 45s and renders the serie board with a LIVE pulse + running score — web via `MatchResults`, native via `TeamResults`. Native polls the same web route (no BITS access in the app). Note: live population depends on the division being live-scored in BITS; degrades to a "waiting for first serie" state. |
