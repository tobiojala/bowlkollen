...
export default function LeaguePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase.from('teams').select('id, name, club').not('name', 'is', null),
      supabase.from('matches').select('id, home_team_id, away_team_id, home_score, away_score, status').eq('status', 'completed').not('home_score', 'is', null),
    ]).then(([{ data: teamsData }, { data: matchesData }]) => {
      if (!teamsData || !matchesData) return setLoading(false)

      const teams = teamsData as Team[]
      const matches = matchesData as Match[]

      // Only include teams that have played at least one match
      ...
      const list = Object.values(table).map(s => ({ ...s, diff: s.ptsFor - s.ptsAgainst }))
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: C.textMuted }}>Elitserien Herrar 2025/2026</div>
          </div>
        ...
        <div style={{ background: C.card, borderRadius: 14, border: '1px solid ' + C.border, overflow: 'hidden' }}>
          ...
          {standings.map((s, i) => {
            const zone = getZone(i, standings.length)
            const prev = i > 0 ? getZone(i - 1, standings.length) : zone
            const showDivider = zone !== prev
            ...
            return (
              <div key={s.team.id}>
                {showDivider && (
                  <div style={{ height: 2, background: zoneColor(zone) + '44', margin: '0' }} />
                )}
                <a href={'/teams/' + s.team.id} style={{
                  display: 'grid',
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingLeft: 4 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 6, background: tclo, border: '1.5px solid ' + tc, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, fontWeight: 900, color: tc, flexShrink: 0, letterSpacing: -0.5 }}>
                  ...
                  <div style={{ ...col, fontSize: 12, color: C.textMuted }}>{s.ptsFor} - {s.ptsAgainst}</div>
