  ...
  return (
    <main style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '24px 16px' }}>
        ...
        {loading ? (
          <div style={{ padding: 32, textAlign: 'center', color: C.textMuted }}>Laddar...</div>
        ) : standings.length === 0 ? (
          ...
          <div style={{ background: C.card, borderRadius: 14, border: '1px solid ' + C.border, overflow: 'hidden' }}>
            ...
            {/* Header */}
            <div style={{ display: 'grid', gridTemplateColumns: '28px 1fr 32px 32px 32px 52px 36px 36px', padding: '9px 12px', background: C.surface, borderBottom: '1px solid ' + C.border }}>
              {['#', 'Lag', 'S', 'V', 'F', 'Total', 'D', 'P'].map((h, i) => (
            ...
            {standings.map((s, i) => {
              ...
              return (
                <div key={s.team.id}>
                  {showDivider(i) && (
                    <div style={{ height: 1, background: zoneColor(i) + '55', margin: 0 }} />
                  )}
                  
                    href={'/teams/' + s.team.id}
                    style={{ display: 'grid', gridTemplateColumns: '28px 1fr 32px 32px 32px 52px 36px 36px', padding: '11px 12px', borderBottom: i < standings.length - 1 ? '1px solid ' + C.border : 'none', borderLeft: '3px solid ' + border, textDecoration: 'none', alignItems: 'center' }}
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 12, flexWrap: 'wrap' }}>
          {[
            { color: C.accent, label: 'SM-slutspel' },
            { color: C.green, label: 'Play-off' },
            { color: '#e05555', label: 'Kvalar' },
            { color: '#666666', label: 'Nedflyttning' },
          ].map(({ color, label }) => (
