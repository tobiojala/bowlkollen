...
export default function AtlasPage() {
  ...
  // Pinch out → enter map mode ("zoom out to see all grids")
  ...
  const onTouchMove = (e: React.TouchEvent) => {
    if (e.touches.length !== 2 || !pinchRef.current || mapMode) return
    const dx = e.touches[0].clientX - e.touches[1].clientX
    const dy = e.touches[0].clientY - e.touches[1].clientY
    if (Math.hypot(dx, dy) / pinchRef.current.dist < 0.72) {
    ...
    >
      ...
      {loading ? (
        ...
        <div style={{ flex: 1, minHeight: 0, position: 'relative' }}>
          ...
          {/* Map overlay — all division grids at once, pinch or button to reach */}
          <AnimatePresence>
            {mapMode && (
              <motion.div
                key="map"
