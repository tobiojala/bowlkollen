...
export default function TillganlighetPage({ params }: Props) {
  ...
  const ResponseGroup = ({ group, label, color, bg: gbg }: { group: any[]; label: string; color: string; bg: string }) => (
    group.length > 0 ? (
          ...
          {group.map(r => {
            ...
            return (
              <div key={r.user_id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: gbg, borderRadius: 10, border: '1px solid ' + color + '33' }}>
                {p?.avatar_url ? (
                  <img src={p.avatar_url} style={{ width: 28, height: 28, borderRadius: '50%', flexShrink: 0, objectFit: 'cover' }} />
                ) : (
          ...
          <div>
            ...
            <div style={{ background: card, borderRadius: 14, border: '1px solid ' + border, padding: '14px 16px', marginBottom: 16 }}>
              ...
              <div style={{ height: 6, borderRadius: 3, background: border, overflow: 'hidden', display: 'flex' }}>
                ...
                {noGroup.length > 0 && <div style={{ width: (noGroup.length / total * 100) + '%', background: '#e24b4a', tran
mkdir -p "src/app/team/[id]/tillganglighet/[matchid]"
cat > "src/app/team/[id]/tillganglighet/[matchid]/page.tsx" << 'EOF'
'use client'
      ...
      <div style={{ maxWidth: 500, margin: '0 auto', padding: '20px 20px 48px' }}>
        ...
        {total > 0 && (
          <div>
            ...
            <ResponseGroup group={yesGroup} label="KAN SPELA" color="#1d9e75" bg={isDark ? 'rgba(29,158,117,0.08)' : 'rgba(29,158,117,0.05)'} />
            <ResponseGroup group={maybeGroup} label="KANSKE" color="#f5c200" bg={isDark ? 'rgba(245,194,0,0.08)' : 'rgba(245,194,0,0.05)'} />
            <ResponseGroup group={noGroup} label="KAN INTE" color="#e24b4a" bg={isDark ? 'rgba(226,75,74,0.08)' : 'rgba(226,75,74,0.05)'} />
          </div>
        )}
