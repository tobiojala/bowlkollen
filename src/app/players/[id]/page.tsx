...
export default function PlayerPage({ params }: Props) {
  ...
  const trendColor = formTrend === '↑' ? '#38a088' : formTrend === '↓' ? '#e05555' : C.textMuted
          ...
          {results.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', marginTop: 20, borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.10)' }}>
              {[
                ...
                { label: 'BÄSTA', value: bestSeries ?? '—', color: '#38a088' },
                ...
                return (
                  ...
                  >
                        ...
                        {games.map((g: number, i: number) => (
                          <React.Fragment key={i}>
                            <span style={{
                              ...
                              color: g >= 250 ? '#ffffff' : g >= 200 ? '#38a088' : C.textMuted,
                    ...
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      <div style={{ fontSize: 22, fontWeight: 900, color: total >= 800 ? '#38a088' : C.accent }}>{total}</div>
