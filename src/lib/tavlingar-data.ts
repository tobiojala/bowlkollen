...
export type TavlingFilter = 'alla' | 'pagaende' | 'kommande' | 'avslutad'

export const TAVLING_SPRING = { type: 'spring', stiffness: 320, damping: 30 } as const
...
export const TAVLINGAR: Tavling[] = [
  ...
  {
    id: 'gp-final-2026',
    name: 'Challenger Grand Prix — Final',
...
export const TAVLING_FILTERS: { key: TavlingFilter; label: string }[] = [
  { key: 'alla', label: 'Alla' },
...
export function tavlingSectionTitle(filter: TavlingFilter): string {
  if (filter === 'alla') return 'ALLA TÄVLINGAR'
  if (filter === 'pagaende') return 'PÅGÅENDE'
  if (filter === 'kommande') return 'KOMMANDE'
  return 'AVSLUTADE'
}
