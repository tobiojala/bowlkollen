  ...
  return (
    <div style={{ marginTop: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '14px 16px 10px', borderBottom: '1px solid ' + C.border }}>
        <span className="section-label" style={{ color: C.textMuted }}>Honor Roll</span>
        <span style={{ fontSize: 9, color: C.textMuted, opacity: 0.6 }}>· senaste 7 dagarna</span>
      </div>
        ...
        {honor.map((e, i) => {
          ...
          if (isPerfect) return (
            <a key={i} href={'/matches/' + e.matchId} style={{
              ...
              <div className="num" style={{ fontSize: 52, lineHeight: 1, color: '#ffffff', textShadow: '0 0 2px #fff, 0 0 10px rgba(255,255,255,0.75), 0 0 28px rgba(255,255,255,0.25)' }}>300</div>
          ...
          if (isHighSeries) return (
            <a key={i} href={'/matches/' + e.matchId} style={{
              ...
              <div className="num" style={{ fontSize: 40, lineHeight: 1, color: '#d8dff0', textShadow: '0 0 8px rgba(205,218,255,0.55), 0 0 22px rgba(175,198,255,0.2)' }}>{e.seriesTotal}</div>
          ...
          return (
            <a key={i} href={'/matches/' + e.matchId} style={{
              ...
              {isElite
                ? <div className="num" style={{ fontSize: 36, lineHeight: 1, color: '#ffffff', textShadow: '0 0 8px rgba(255,255,255,0.55), 0 0 22px rgba(255,255,255,0.18)' }}>{e.score}</div>
                : <div className="num" style={{ fontSize: 32, lineHeight: 1, color: '#f5c200', textShadow: '0 0 8px rgba(245,194,0,0.5), 0 0 20px rgba(245,194,0,0.2)' }}>{e.score}</div>
              }
