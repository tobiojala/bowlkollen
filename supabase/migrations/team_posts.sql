...
CREATE INDEX IF NOT EXISTS team_posts_team_idx ON public.team_posts (bits_team_id, created_at DESC);

ALTER TABLE public.team_posts ENABLE ROW LEVEL SECURITY;
-- Read: verified members of the team only. Writes go through create_team_post.
DROP POLICY IF EXISTS team_posts_read ON public.team_posts;
CREATE POLICY team_posts_read ON public.team_posts FOR SELECT USING (
  EXISTS (SELECT 1 FROM team_claims tc WHERE tc.user_id = auth.uid() AND tc.bits_team_id = team_posts.bits_team_id AND tc.status = 'verified')
);

-- Per-member "last seen" marker → drives the unread badge.
CREATE TABLE IF NOT EXISTS public.team_post_seen (
  user_id      uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  bits_team_id integer NOT NULL,
  seen_at      timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (user_id, bits_team_id)
);
ALTER TABLE public.team_post_seen ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS team_post_seen_own ON public.team_post_seen;
CREATE POLICY team_post_seen_own ON public.team_post_seen FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
BEGIN
  ...
  INSERT INTO team_posts (bits_team_id, author_user_id, title, body)
...
LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  INSERT INTO team_post_seen (user_id, bits_team_id, seen_at)
...
-- ─── get_my_unread_posts ─────────────────────────────────────────────────────
-- Per-team unread count for the caller — powers the badge on Profil / Mitt lag.
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  FROM team_claims tc
  LEFT JOIN team_post_seen s ON s.user_id = tc.user_id AND s.bits_team_id = tc.bits_team_id
  LEFT JOIN team_posts p ON p.bits_team_id = tc.bits_team_id
